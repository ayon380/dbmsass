  ...
  return (
    <main className="container mx-auto my-8 p-4 relative">
      <h1 className="text-3xl font-bold text-center mb-8 relative z-10 ">
        <span className="flowing-background">DBMS Assignment</span>
      </h1>
      <div className="flex justify-around mb-8">
        <button
          className={`px-4 py-2 rounded-full transition duration-300 ease-in-out ${page === 'book' ? 'bg-blue-500 text-white' : 'bg-gray-300 hover:bg-blue-300 hover:text-white'}`}
          onClick={() => setPage('book')}
        >
          Book
        </button>
        <button
          className={`px-4 py-2 rounded-full transition duration-300 ease-in-out ${page === 'ecom' ? 'bg-blue-500 text-white' : 'bg-gray-300 hover:bg-blue-300 hover:text-white'}`}
